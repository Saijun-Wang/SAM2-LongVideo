...
def init_propagate_state():
    global inference_state, use_pretrained_YOLO, labels1, points1, labels2, points2, ax, wrong, wrong_subsequence, save_yolo_results
    if subsequence == 1 and use_pretrained_YOLO == False:
        ...
        for index, file_name in enumerate(os.listdir(os.path.join(target_dir, f'folder_{subsequence}'))):
            ...
            for result in results:
                ...
                # print("bbox:", bbox)
                if bbox.size(0) >= 2:
                    top_conf, top_index = torch.topk(conf, 2)
                    if top_conf[0] > config["confidence_threshold"] and top_conf[1] > config["confidence_threshold"]:
                        bbox1 = bbox[top_index[0]]
                        yolo_bbox1 = bbox1.cpu().numpy()
                        yolo_conf1 = top_conf[0]
                        bbox2 = bbox[top_index[1]]
                        yolo_bbox2 = bbox2.cpu().numpy()
                        yolo_conf2 = top_conf[1]
                        break
